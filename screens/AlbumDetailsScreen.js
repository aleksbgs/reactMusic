...
export default class AlbumDetailsScreen extends React.Component {
    ...
    componentDidMount() {
        ...
        action.getAlbumTracks(album.id).then(tracks => {
            this.setState({ tracks });
        })
            .catch(error => console.error(error))
    }
    async saveTrackToFavorite(album, track) {
        const favoriteAlbums = await action.retrieveData('favoriteAlbums') || {};
        ...
        const success = await action.storeData('favoriteAlbums', favoriteAlbums);
        if (success) {
            Alert.alert(
                'Track Added!',
                    ...
                    <View style={styles.header}>
                        <View>
                            <Avatar style={styles.avatar}
                                xlarge rounded source={{ uri: album.cover_medium }}>
